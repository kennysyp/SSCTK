...
class Up1(nn.Module):    # 上采样
    def __init__(self, in_channels, out_channels):
        ...
        self.upsampling = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2) # 转置卷积
        self.conv = DoubleConv(2432, out_channels)
...
def window_partition(x, window_size):
    """
    Args:
        x: (B, H, W, C)(16,224,224,3)
        window_size (int): window size

    Returns:
        windows: (num_windows*B, window_size, window_size, C)(16*4,56,56,3)
    """
    B, H, W, C = x.shape #(B,224,224,3)
    x = x.view(B, H // window_size, window_size,#(B,32,7,32,7,3)
               W // window_size, window_size, C)
    windows = x.permute(0, 1, 3, 2, 4, 5).contiguous(   #(B,32,32,7,7,3)
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        #卷积部分
        self.in_conv = DoubleConv(in_chans, 64)
    ...
    def forward(self, x):
        ...
        #print(x.shape)
        #print(x6.shape)
        xre = rearrange(x,'b (h w) c -> b c h w',h=7,w=7)
        #print(xre.shape)
        x6 = torch.cat([xre, x6], dim=1)
        #decoder
        #unet
        xu = self.up1(x6,x5)
        xu = self.up2(xu, x4)
