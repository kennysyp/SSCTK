...
from networks.fct import FCT
import argparse
from networks.vision_transformer import SwinUnet as ViT_seg
from networks.config import get_config
from networks.nnunet import initialize_network
from networks.FFCUNet3D import get_ffc_unet_3d
from networks.FUnet import FUNet
...
def net_factory(net_type="unet", in_chns=1, class_num=3):
    if net_type == "unet":
        ...
        net = FCT()
    elif net_type == "unet_3D":
        net = unet_3D(n_classes=class_num, in_channels=in_chns)
    elif net_type == "swin_unetr":
        net = SwinUNETR(img_size=(128, 128, 128),in_channels=in_chns,out_channels=class_num)
    elif net_type == "ffcunet_3D":
        net = get_ffc_unet_3d(n_channels=in_chns,n_classes=class_num)
    elif net_type == "funet":
        net = FUNet(in_ch=in_chns,out_ch=class_num)
    else:
        net = None
    return net
