...
class ContrastiveLoss(torch.nn.Module):  
    ...
    def forward(self, features1, features2):  
        features1 = self._flatten_features(features1)  
        features2 = self._flatten_features(features2)  
        ...
        similarities = torch.mm(features1, features2.t())  

        labels = torch.eye(batch_size, device=features1.device)  

        pos_loss = (1 - similarities.diag()).pow(2)  

        neg_loss = F.relu(similarities - self.margin * (1 - labels)).pow(2)  

        # 合并损失  
        loss = (pos_loss.sum() + neg_loss.sum()) / (batch_size * batch_size)  
    ...
    def _flatten_features(self, features):  
        # 如果特征不是 2D，将其展平为 2D  
        if features.dim() > 2:  
            return features.contiguous().view(features.size(0), -1)  
        return features 
